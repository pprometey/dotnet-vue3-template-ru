...
Веб-приложение: SPA на Vue 3 и модульный монолит на .NET 10 (Clean Architecture, CQRS через Wolverine), PostgreSQL, локальный стек одной командой через .NET Aspire.

Замените этот абзац описанием своего продукта. Устройство системы - [docs/architecture.md](docs/architecture.md). Обоснования решений - [docs/adr/](docs/adr/README.md).

Проект развёрнут из шаблона монорепозитория .NET 10 + Vue 3.

## Стадия

Предметных модулей ещё нет: `Ping` и `Notes` - сквозные срезы, доставшиеся от шаблона. Они показывают весь путь от формы в SPA до строки в PostgreSQL и держат часть тестов инфраструктуры.

Удалять их сразу нельзя: на `Notes` завязаны 16 из 26 интеграционных тестов и якоря сборок в архитектурных тестах, а `Ping` подключает сборку Application к обнаружению хендлеров Wolverine. Порядок вывода `Notes` при появлении первого реального модуля - [docs/guides/retire-demo-slice.md](docs/guides/retire-demo-slice.md).
...
Все порты закреплены жёстко, поэтому два приложения с одинаковыми значениями одновременно не поднимутся. Если рядом поднимается ещё одно приложение на этом же стеке, разведите порты явно - каждый из них задан в одном месте, см. таблицу выше.
